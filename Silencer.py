...
df_dist = pd.read_csv(f"silencer.noloop.nearTSS.bed", sep="\t", header=None)
...
plt.ylim(0, 135)
plt.savefig(f"8.19.silencer.pdf")

df = df.rename({"dist": "dist(Kb)"}, axis=1)
df.to_csv("gene.noloop_silencer.dist_group.csv", sep="\t", index=False)
